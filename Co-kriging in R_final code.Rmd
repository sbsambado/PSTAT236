...
grid <- readOGR("~/PSTAT236/grid_bay_26910.shp")
frog <- readOGR("~/PSTAT236/dat_bay_26910.shp")

grid_coord <- as.data.frame(grid@coords)

gridded (grid_coord) = ~coords.x1 + coords.x2

frog@data[,39] = as.numeric(frog@data[,39])
frog@data[,40] = as.numeric(frog@data[,40])
frog@data[,41] = as.numeric(frog@data[,41])

ze_1 = variogram(log(ZE_1) ~ 1, frog)
ze_2 = variogram(log(ZE_2) ~ 1, frog)
ze_3 = variogram(log(ZE_3) ~ 1, frog)
